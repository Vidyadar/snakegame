...
import { addScore, getPlayerBestScore } from "@/lib/firestore"
...
export default function HomePage() {
  ...
  useEffect(() => {
    const loadHighScore = async () => {
      ...
      try {
        const firebaseHighScore = await getPlayerBestScore(user.uid)
  ...
  const endGame = async (finalScore: number) => {
    ...
    if (finalScore > 0 && user) {
      try {
        await addScore(user.uid, displayName, finalScore)
      } catch (error) {
        ...
        const filtered = scores.filter((entry: any) => entry.id !== user.uid)
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-2 sm:p-4">
      ...
      {/* Game Over Modal for NFT Minting */}
